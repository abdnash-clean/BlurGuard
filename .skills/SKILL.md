...
- core/processing/     GPU blur/pixelate/mask rendering, watermark, metadata strip on export
...
- feature/gallery/     gallery browse + post-capture manual edit + face grouping
...
- feature/* MUST NOT depend on another feature/*, nor directly on core/ml, core/tracking, core/processing, core/camera, core/recognition.
- Features access the engine ONLY through core/domain use cases.
- core/domain may orchestrate camera/processing/ml/tracking/recognition/data.
- Engine modules (camera, processing, ml, tracking, recognition) depend only on core/model and core/common, and talk to each other ONLY via interfaces declared in core/model.
...
3. ON-DEVICE ONLY: no inference, embedding, or footage may leave the device. Do not add network calls in the recording/processing path.
4. OFFLINE-ONLY enforced centrally in app/, not per-feature.
5. METADATA STRIP (GPS/device/timestamp) is a mandatory, default-on export stage in core/processing.
...
- New camera/recording behavior -> core/camera (capture) and/or core/processing (pixels)
...
- Orchestration / business decision spanning ml+tracking+recognition+processing -> core/domain use case
